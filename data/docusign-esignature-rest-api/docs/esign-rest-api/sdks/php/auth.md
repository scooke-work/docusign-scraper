...
source_url: https://developers.docusign.com/docs/esign-rest-api/sdks/php/auth/
...
## Authorization Code Grant with PHP
...
- **Integration key** (client ID): This can be obtained in your developer account by accessing the
  ...
  [Administration Guide](https://support.docusign.com/s/document-item?language=en_US&bundleId=pik1583277475390&topicId=pmp1583277397015.html) for more information.
...
- **Redirect/Callback URI**: This is the URI to which the end user will be redirected when your application uses the Authorization Code Grant flow. That URI must exactly match one of the redirect URIs you define for your integration key on the
...
Authorization Code Grant requires a web interface because users must utilize the Docusign Identity Provider (IdP) to authenticate with their Docusign account. In this section, we’ll cover the recommended practices for using Authorization Code Grant in your PHP application. Other PHP web technologies may require a different technique to complete the Authorization Code Grant flow and are beyond the scope of this guide.

### Using Authorization Code Grant in your PHP applications

In your client, you can instantiate an
`OAuth` object to enable the Docusign Authorization Code Grant flow to work with your application. You can find a full working example in our
[GitHub repository](https://github.com/docusign/code-examples-php/blob/master/src/Services/CodeGrantService.php).
...
```
    function get_oauth_provider(): Docusign
    {
     return new Docusign([
      ...
      'redirectUri' => $GLOBALS['DS_CONFIG']['app_url'] .
                       '/index.php?page=ds_callback',
...
Call the following code from your client router service:

```
    if ($_SESSION['auth_service'] == "CodeGrantService") {
        $this->authService = new CodeGrantService();
    }
```

## JSON Web Tokens (JWT) with PHP

Every PHP SDK (eSignature as well as others) includes two JWT Grant methods:

- **[requestJWTUserToken](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Client/ApiClient.php#L556)**: This method uses the JWT Grant flow to request a token that will represent a specific user. This JWT Grant flow is used for all eSignature REST API methods and for most other Docusign API methods as well.
- **[requestJWTApplicationToken](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Client/ApiClient.php#L617)**: This method uses the JWT Grant flow to request a token that will represent the application itself (not a user). This JWT Grant flow is useful for specific Docusign partners who are part of special Docusign programs allowing them to manage accounts using the [Docusign Admin API](https://developers.docusign.com/docs/admin-api/).
...
- **RSA private key**: This is for the integration key you obtained above and can also be created in the [Apps and Keys](https://admindemo.docusign.com/authenticate?goTo=appsAndKeys) page. You only need the private key, and it can only be copied once, if you used the administration tool to create it. Make sure to retain it for your records.
- **Base path**: To use either JWT Grant method, you need to use an SDK `ApiClient` object. To create an `ApiClient` object, you need to provide a base path. Since the application’s correct base path is not yet known, use placeholder values to indicate which system should be queried:
...
- **Parameter formats and additional parameters** are documented in the [PHP SDK](https://docusign.github.io/docusign-esign-php-client/).
...
To use JWT from the PHP SDK, you simply have to make the following call (you can find this code in [GitHub](https://github.com/docusign/code-examples-php/blob/master/src/Services/JWTService.php#L64)):
...
The
[getUserInfo](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Client/ApiClient.php#L665) function returns a
[UserInfo](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Client/Auth/UserInfo.php#L36) object that has various pieces of information related to the user. The
[accounts](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Client/Auth/UserInfo.php#L55) property is a list of
[Account](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Client/Auth/Account.php#L43) objects that provides the collection of Docusign accounts and their information. You can use the
`is_default` property of the
`Account` object to check if this is the user’s default account. The
`UserInfo` object also includes the
`name` and
...
[setBasePath](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Client/Auth/OAuth.php#L103) method to ensure the correct base path is set.
...
The basic flow of instantiating the
`ApiClient` object looks like this:

```
    $config = new Configuration();
    $config->setHost($args['base_path']);
    $config->addDefaultHeader('Authorization', 'Bearer ' .
                             $args['ds_access_token']);
...
The
[addDefaultHeader](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Configuration.php#L317) method sets the
[defaultHeaders](https://github.com/docusign/docusign-esign-php-client/blob/master/src/Configuration.php#L84) variable of the
`Configuration` class. The
`defaultHeaders` variable is an associative array that is used to provide additional HTTP headers for API requests. You can add additional headers to be used with API calls that use the
...
`ds_access_token` variable. This only needs to be done once for all API calls that will be utilizing the same
...
This code is used to instantiate a new
