...
- [Send a draft envelope](https://developers.docusign.com/docs/esign-rest-api/reference/envelopes/envelopes/update/#sending-a-draft-envelope)
- [Void an in-process envelope](https://developers.docusign.com/docs/esign-rest-api/reference/envelopes/envelopes/update/#voiding-an-in-process-envelope)
- [Modify a draft envelope](https://developers.docusign.com/docs/esign-rest-api/reference/envelopes/envelopes/update/#modifying-envelope-email-information)
- [Purge documents and envelope metadata from the Docusign platform](https://developers.docusign.com/docs/esign-rest-api/reference/envelopes/envelopes/update/#purging-documents-from-docusign)
...
When the purge request is initiated, the items to be purged are placed in the purge queue for deletion in prior to actual purging. As a default configuration in the sender’s account, the sender and all recipients with Docusign accounts associated with the envelope get an email notification the documents will be deleted when the envelope first enters the purge queue (e.g., in 14 days prior to purge, under normal purge queue processing conditions). The notification contains a link to the documents, which allows recipients to download a document before it is purged. A second email notification is additionally sent generally 7 days later prior to expiration of the purge queue period as another reminder of the upcoming purge. At the end of the purge queue period, the documents are deleted from the system. Recipients without Docusign accounts do not receive email notifications.
...
| Query Parameters |  |  |
| --- | --- | --- |
| advanced\_update | string | When **true,** allows the caller to update recipients, tabs, custom fields, notification, email settings and other envelope attributes. |
| recycle\_on\_void | string |  |
| resend\_envelope | string | When **true,** sends the specified envelope again. |
